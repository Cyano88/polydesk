...
function executionHandoff(
  ...
  return {
    ...
    tradePreflight: input.side === 'BUY' ? {
      ...
      approvalBoundary: 'Deposit wallets use deposit-wallet relayer approvals. Never use the legacy proxy factory. Negative-risk buys require the CLOB adapter allowance as well as exchange allowance. Unlimited approval requires separate explicit consent; verify its receipt and refresh CLOB before resuming.',
