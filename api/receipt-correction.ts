...
type Correction={schema:'polydesk-receipt-correction-v1';transaction:string;originalAnalysisHash:string;originalResultHash:string;originalResult:unknown;issue:string;createdAt:string;status:'REQUESTED'|'PUBLISHED';addendum?:Addendum;revisionHash?:string;publishedAt?:string}
...
export class ReceiptCorrections {
 ...
 async request(tx:string,issue:string){const p=await this.original(tx);if(typeof issue!=='string'||!issue.trim()||issue.length>5000)throw new CorrectionError(400,'INVALID_ISSUE');return this.store.mutate(key(tx),c=>{if(c){if(c.issue!==issue||c.originalAnalysisHash!==p.analysisHash)throw new CorrectionError(409,'CORRECTION_CONFLICT');return c}return {schema:'polydesk-receipt-correction-v1',transaction:tx.toLowerCase(),originalAnalysisHash:p.analysisHash!,originalResultHash:digest(p.response),originalResult:p.response,issue,createdAt:new Date().toISOString(),status:'REQUESTED'}})}
 async publish(tx:string,originalHash:string,raw:unknown){const p=await this.original(tx),a=validateAddendum(raw);return this.store.mutate(key(tx),c=>{if(!c)throw new CorrectionError(404,'CORRECTION_NOT_FOUND');if(c.originalAnalysisHash!==originalHash||p.analysisHash!==originalHash||digest(p.response)!==c.originalResultHash)throw new CorrectionError(409,'ORIGINAL_CHANGED');const revisionHash=digest({transaction:c.transaction,originalAnalysisHash:c.originalAnalysisHash,addendum:a});if(c.status==='PUBLISHED'){if(c.revisionHash!==revisionHash)throw new CorrectionError(409,'REVISION_IMMUTABLE');return c}return {...c,status:'PUBLISHED',addendum:a,revisionHash,publishedAt:new Date().toISOString()}})}
}
...
export function createReceiptCorrectionRouter(service=new ReceiptCorrections(),authorize=correctionOperator){const router=Router({mergeParams:true});router.use((_req,res,next)=>{res.setHeader('Cache-Control','no-store');next()});router.all('/',async(req,res)=>{try{
 ...
 if(b.action==='REQUEST'&&Object.keys(b).every(k=>['action','issue'].includes(k)))c=await service.request((req.params as Record<string,string>).transaction,b.issue)
 else if(b.action==='PUBLISH'&&Object.keys(b).every(k=>['action','originalAnalysisHash','addendum'].includes(k)))c=await service.publish((req.params as Record<string,string>).transaction,b.originalAnalysisHash,b.addendum)
