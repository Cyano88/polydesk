...
Live validation also exposed a third reproducible defect: addSmartTraderReplaySchema encoded PAYMENT-REQUIRED as unpadded URL-safe Base64. Coinbase's validator requires standard Base64 JSON and rejected the new challenge as undecodable. Earlier challenge bytes happened to pass; metadata shape changes made the encoding defect observable. Fixed the shared Smart Trader helper to emit padded standard Base64, preserving the exact decoded business schema and payment terms. Added alphabet/padding/round-trip regression coverage. The failed validation response is preserved in BASE_DISCOVERY_BASE64_FAILURE_20260911.json.
