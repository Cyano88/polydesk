...
These routes require no wallet login or payment. Partner-scoped free discovery jobs are separately available at /api/v1/jobs with provisioned credentials; see [partner jobs](PARTNER_JOBS.md). Free discovery jobs do not buy research. Partner research reservations are separately available at /api/v1/research-jobs and bind to verified Base x402 payment; see [paid research](PARTNER_PAID_RESEARCH.md). Subscriptions, trade submission and operator controls are not v1 operations. MCP is not implemented. Unknown/repeated query parameters are rejected; an upstream failure never triggers a paid fallback. Existing paid routes keep their original contracts.
