...
signing step. For DEPOSIT_WALLET, verify pUSD allowance to the chosen V2 exchange
and, for negative-risk buys, the CLOB-required Neg Risk Adapter. Never repair
through the legacy proxy factory. The relayer rejected bounded adapter approval
and required MaxUint256; unlimited permission needs explicit separate buyer
consent and must never be inferred from the order spend cap. After a confirmed
approval, refresh CLOB and read the exact on-chain allowance. Missing approvals
require a separately previewed deposit-wallet relayer
...
### Managed buyer-local FOK BUY handoff

Use conversation action PREPARE_LOCAL_TRADE with trade fields ownerAddress, marketSlug, outcome, maxTotalUsdc, limitPrice, and acknowledgeIndependentDecision=true only after the buyer chooses an independent trade. The response includes the rounded exact order, all public readiness checks, and an owner signing message. A preview is not permission to sign or execute. Do not substitute the watched wallet for the trading owner.

Return the order artifact to the buyer runtime. On the buyer's reviewed Windows/WSL installation, save {order, signature} after the owner signs the returned authorizationMessage. The buyer can inspect the message without wallet access with `npm run managed-agent:local -- --request ORDER_FILE`. Only an explicitly approved exact trade may use `--execute`. Run it on the buyer's machine and wallet, never the provider VPS. Do not send reusable wallet credentials or keys through A2A.

The adapter verifies the original owner signature, active exact subscription, current local Polygon owner, and fresh unchanged fee-inclusive preparation. It uses the existing shared durable execution guard. The separate hash-pinned managed native binary checks exact owner, deposit wallet, token, condition, FOK policy, total cap, size, price and expiry before signing and again before submission. It prohibits automatic collateral wrapping and does not request approvals. Missing or changed readiness requires a new preview; never widen the signed order.
...
## Mandatory buyer-local memory continuation
The supported buyer launcher now requires Sibyl/live-position reconciliation before every live BUY/SELL. See docs/MEMORY_DEPENDENT_BUYER_FLOW_20260910.md. On EXISTING_EXPOSURE_REVIEW_REQUIRED show the existing exposure and exact additional buy, obtain buyer approval, then pass the returned digest with -MemoryReviewDigest and unchanged arguments. Digest acknowledgement never grants trading authority. On mismatch or unavailable memory, reconcile the existing fills/position; never bypass the launcher or retry a trade to repair memory. -CheckMemoryContinuation is read-only and may be used without spending. Keep buyer credentials on the buyer host.
