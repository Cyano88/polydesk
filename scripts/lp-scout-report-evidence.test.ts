...
import { lpScoutEvidenceView, lpVerificationStatus } from '../api/lp-scout-report-evidence.js'
...
test('legacy timed-out queued record is not presented as ongoing verification',()=>{
 assert.equal(lpVerificationStatus(false,false,{retryable:true}).aiStatus,'needs_attention')
 assert.equal(lpVerificationStatus(false,true,{}).aiStatus,'needs_attention')
 assert.equal(lpVerificationStatus(false,false,{}).aiStatus,'pending')
 assert.equal(lpVerificationStatus(true,true,{retryable:true}).aiStatus,'complete')
})
