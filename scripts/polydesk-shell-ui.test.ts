...
test('workspace navigation keeps four primary destinations and gives Portfolio four compact sections', () => {
  ...
  assert.match(layout, /workspace === 'agent'[\s\S]*'relative z-40 shrink-0'[\s\S]*'fixed inset-x-0 bottom-0 z-50'/)
...
test('PolyDesk Agent uses the extracted modern composer without category selection', () => {
  ...
  assert.match(polyDeskPage, /PolyDesk Agent is live on OKX AI for agents to use or resell\./)
  ...
  assert.doesNotMatch(layout, /pb-\[var\(--polydesk-footer-height\)\]/)
  ...
  assert.match(layout, /const viewportReduction = settledViewportHeight - viewport\.height/)
  assert.match(layout, /setMobileKeyboardOpen\(viewportReduction > 120\)/)
  assert.match(layout, /Math\.max\(settledViewportHeight, viewport\.height\)/)
...
test('Portfolio consolidates balance, owned actions and watched-market trading', () => {
  ...
  assert.match(paymentLinks, /\{ key: 'monitor', label: 'Monitor LP'/)
...
test('public portfolio watches verify email ownership and use authoritative market transitions', () => {
  ...
  assert.match(paymentLinks, /label: 'Monitor LP'/)
