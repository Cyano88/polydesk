...
try {
 for (const key of ['--market-slug', '--outcome', '--shares', '--price']) if (!args.includes(key)) throw new Error(`Missing ${key}`)
 ...
 const response = await fetch('https://polydesk.trade/api/polymarket-account/sell-preflight', {
 method: 'POST', headers: { 'Content-Type': 'application/json' }, signal: AbortSignal.timeout(30_000),
 body: JSON.stringify({ ownerAddress: addresses[0].address, marketSlug: get('--market-slug'), outcome: get('--outcome'), shares: get('--shares'), minimumPrice: get('--price'), orderType: 'FOK' }) })
