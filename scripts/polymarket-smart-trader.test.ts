...
test('PREPARE returns a preview-only official plugin handoff and performs no signing', async () => {
  ...
  assert.equal(result.data.handoff.fundingFlow?.requiredBalanceUsdc, null)
  assert.equal(result.data.handoff.tradePreflight?.endpoint, '/api/polymarket-account/trade-preflight')
  assert.equal(result.data.handoff.tradePreflight?.input.maxTotalUsdc, 5)
  assert.equal(result.data.handoff.tradePreflight?.input.postOnly, true)
