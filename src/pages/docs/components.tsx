...
export function DocHeader({ title, description }: { title: string; description: string }) {
  return (
    <header className="border-b border-gray-200 pb-8">
      ...
      <p className="mt-3 text-lg leading-8 text-gray-600">{description}</p>
    </header>
  )
}
    ...
    <div className="overflow-x-auto rounded-xl border border-gray-200">
      <table className="w-full text-left text-sm">
        <thead className="bg-gray-50">
          <tr>{headers.map(header => <th key={header} className="whitespace-nowrap px-4 py-3 font-semibold text-gray-700">{header}</th>)}</tr>
        </thead>
        <tbody className="divide-y divide-gray-200">
          {rows.map((row, index) => (
            <tr key={index}>
              {row.map((cell, cellIndex) => <td key={cellIndex} className="px-4 py-3 align-top text-gray-600">{cell}</td>)}
            </tr>
          ))}
        </tbody>
      </table>
...
export function Note({ children }: { children: ReactNode }) {
  return <div className="rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm leading-6 text-gray-700">{children}</div>
}
